...
SubsetRegist=function(clean.regist){
  # getting deadline
  deadline = max(clean.regist$time) - as.difftime(28,units='days') # can be modified
  
  subset.regist = clean.regist[which(clean.regist$time<deadline),]
...
# 
# 
# # cc=table(data_regist_multiact$retention,summary.data_action_multiact)
# 
# 
# rate_mltpact = sum(label_mltpact)/5000
# 
...
# 
# 
# # conclusion: from June 26 to July 02, regist time all 0 hour => Problematic
# # conclusion: from June 26 to July 02, action time all 0 hour => Problematic
# # runed following code as a checking for the above fact
# # data_action$time[ii]$hour
# # which(data_action$time[ii]$hour != 0)
# # plot(data_action$time$hour)
# # plot(data_regist$hour)
# 
